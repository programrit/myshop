...
if(isset($_POST["otp_verify"])){
    ...
    if($row["otp_verify"]==1){
        ...
        if($current_time>=$check_time){
            $update="UPDATE user SET otp_verify='2'  WHERE email='$email'";
            ...
            if($otp==$row["otp"]){
                $updated="UPDATE user SET otp_verify='3' WHERE email='$email'";
                if($conn->query($updated)==TRUE){
                    $msg1="OTP verfication successfully!";
                    header("refresh:3; url=set-password?email=".base64_encode(strrev($email)));
                }else{
                    $msg="Something went wrong.Please try again later!";
                    // header("refresh:2; url=otp-verify");
                }
...
<body>
    ...
    <!-- Contact Start -->
    <?php include('__template/__otp_verify.php'); ?>
    <!-- Contact End -->
